import React from "react";
import p1 from "../assets/dark/imgs/portfolio/3/1.jpg";
import p2 from "../assets/dark/imgs/portfolio/3/2.jpg";
import p3 from "../assets/dark/imgs/portfolio/3/3.jpg";
const Portfolio = () => {
  return (
    <>
      <section className="portfolio section-padding">
        <div className="container">
          <div className="sec-lg-head mb-80">
            <div className="row">
              <div className="col-lg-8">
                <div className="position-re">
                  <h6 className="dot-titl colorbg-3 mb-10">پروژه های منتخب</h6>
                  <h2 className="fz-60 fw-700">آثار برجسته</h2>
                </div>
              </div>
              <div className="col-lg-4 d-flex align-items-center">
                <div className="text">
                  <p>
                    لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ، و
                    با استفاده از طراحان گرافیک است، چاپگرها{" "}
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="work-row">
            <div className="item bord-thin-top wow fadeIn" data-wow-delay=".3s">
              <div className="row">
                <div className="col-lg-3 d-flex align-items-end">
                  <div className="title">
                    <span className="numb">01.</span>
                    <h5>لامپ زیبا</h5>
                  </div>
                </div>
                <div className="col-lg-4">
                  <div className="img md-mb30">
                    <img src={p1} alt="" />
                  </div>
                </div>
                <div className="col-lg-5 d-flex align-items-end">
                  <div className="cont">
                    <p>
                      لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ،
                      و با استفاده از طراحان گرافیک است، چاپگرها و{" "}
                    </p>
                    <a
                      href="project-details1.html"
                      className="mt-30 ls1 sub-title"
                    >
                      نمایش پروژه{" "}
                      <i className="ml-5">
                        <svg
                          width="18"
                          height="18"
                          viewBox="0 0 18 18"
                          className="mirror"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M13.922 4.5V11.8125C13.922 11.9244 13.8776 12.0317 13.7985 12.1108C13.7193 12.1899 13.612 12.2344 13.5002 12.2344C13.3883 12.2344 13.281 12.1899 13.2018 12.1108C13.1227 12.0317 13.0783 11.9244 13.0783 11.8125V5.51953L4.79547 13.7953C4.71715 13.8736 4.61092 13.9176 4.50015 13.9176C4.38939 13.9176 4.28316 13.8736 4.20484 13.7953C4.12652 13.717 4.08252 13.6108 4.08252 13.5C4.08252 13.3892 4.12652 13.283 4.20484 13.2047L12.4806 4.92188H6.18765C6.07577 4.92188 5.96846 4.87743 5.88934 4.79831C5.81023 4.71919 5.76578 4.61189 5.76578 4.5C5.76578 4.38811 5.81023 4.28081 5.88934 4.20169C5.96846 4.12257 6.07577 4.07813 6.18765 4.07812H13.5002C13.612 4.07813 13.7193 4.12257 13.7985 4.20169C13.8776 4.28081 13.922 4.38811 13.922 4.5Z"
                            fill="currentColor"
                          ></path>
                        </svg>
                      </i>
                    </a>
                  </div>
                </div>
              </div>
              <div className="main-marq xlrg">
                <div className="slide-har st1 strok">
                  <div className="box">
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                  </div>
                  <div className="box">
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                    <div className="item">
                      <h4>بسته قرمز</h4>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="item bord-thin-top wow fadeIn" data-wow-delay=".3s">
              <div className="row">
                <div className="col-lg-3 d-flex align-items-end">
                  <div className="title">
                    <span className="numb">02.</span>
                    <h5>چراغ دیواری</h5>
                  </div>
                </div>
                <div className="col-lg-4">
                  <div className="img md-mb30">
                    <img src={p2} alt="" />
                  </div>
                </div>
                <div className="col-lg-5 d-flex align-items-end">
                  <div className="cont">
                    <p>
                      لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ،
                      و با استفاده از طراحان گرافیک است، چاپگرها و{" "}
                    </p>
                    <a
                      href="project-details1.html"
                      className="mt-30 ls1 sub-title"
                    >
                      نمایش پروژه{" "}
                      <i className="ml-5">
                        <svg
                          width="18"
                          height="18"
                          viewBox="0 0 18 18"
                          className="mirror"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M13.922 4.5V11.8125C13.922 11.9244 13.8776 12.0317 13.7985 12.1108C13.7193 12.1899 13.612 12.2344 13.5002 12.2344C13.3883 12.2344 13.281 12.1899 13.2018 12.1108C13.1227 12.0317 13.0783 11.9244 13.0783 11.8125V5.51953L4.79547 13.7953C4.71715 13.8736 4.61092 13.9176 4.50015 13.9176C4.38939 13.9176 4.28316 13.8736 4.20484 13.7953C4.12652 13.717 4.08252 13.6108 4.08252 13.5C4.08252 13.3892 4.12652 13.283 4.20484 13.2047L12.4806 4.92188H6.18765C6.07577 4.92188 5.96846 4.87743 5.88934 4.79831C5.81023 4.71919 5.76578 4.61189 5.76578 4.5C5.76578 4.38811 5.81023 4.28081 5.88934 4.20169C5.96846 4.12257 6.07577 4.07813 6.18765 4.07812H13.5002C13.612 4.07813 13.7193 4.12257 13.7985 4.20169C13.8776 4.28081 13.922 4.38811 13.922 4.5Z"
                            fill="currentColor"
                          ></path>
                        </svg>
                      </i>
                    </a>
                  </div>
                </div>
              </div>
              <div className="main-marq xlrg">
                <div className="slide-har st1 strok">
                  <div className="box">
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                  </div>
                  <div className="box">
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                    <div className="item">
                      <h4>چراغ دیواری</h4>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div
              className="item bord-thin-top bord-thin-bottom wow fadeIn"
              data-wow-delay=".3s"
            >
              <div className="row">
                <div className="col-lg-3 d-flex align-items-end">
                  <div className="title">
                    <span className="numb">03.</span>
                    <h5>اجاق های زیبا</h5>
                  </div>
                </div>
                <div className="col-lg-4">
                  <div className="img md-mb30">
                    <img src={p3} alt="" />
                  </div>
                </div>
                <div className="col-lg-5 d-flex align-items-end">
                  <div className="cont">
                    <p>
                      لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ،
                      و با استفاده از طراحان گرافیک است، چاپگرها و{" "}
                    </p>
                    <a
                      href="project-details1.html"
                      className="mt-30 ls1 sub-title"
                    >
                      نمایش پروژه{" "}
                      <i className="ml-5">
                        <svg
                          width="18"
                          height="18"
                          viewBox="0 0 18 18"
                          className="mirror"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M13.922 4.5V11.8125C13.922 11.9244 13.8776 12.0317 13.7985 12.1108C13.7193 12.1899 13.612 12.2344 13.5002 12.2344C13.3883 12.2344 13.281 12.1899 13.2018 12.1108C13.1227 12.0317 13.0783 11.9244 13.0783 11.8125V5.51953L4.79547 13.7953C4.71715 13.8736 4.61092 13.9176 4.50015 13.9176C4.38939 13.9176 4.28316 13.8736 4.20484 13.7953C4.12652 13.717 4.08252 13.6108 4.08252 13.5C4.08252 13.3892 4.12652 13.283 4.20484 13.2047L12.4806 4.92188H6.18765C6.07577 4.92188 5.96846 4.87743 5.88934 4.79831C5.81023 4.71919 5.76578 4.61189 5.76578 4.5C5.76578 4.38811 5.81023 4.28081 5.88934 4.20169C5.96846 4.12257 6.07577 4.07813 6.18765 4.07812H13.5002C13.612 4.07813 13.7193 4.12257 13.7985 4.20169C13.8776 4.28081 13.922 4.38811 13.922 4.5Z"
                            fill="currentColor"
                          ></path>
                        </svg>
                      </i>
                    </a>
                  </div>
                </div>
              </div>
              <div className="main-marq xlrg">
                <div className="slide-har st1 strok">
                  <div className="box">
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                  </div>
                  <div className="box">
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                    <div className="item">
                      <h4>اجاق های زیبا</h4>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default Portfolio;
